...
const defaultElementFactory_1 = tslib_1.__importDefault(require("./elements/defaultElementFactory"));
const Stateful_1 = tslib_1.__importDefault(require("./utils/Stateful"));
...
class Visualization extends Stateful_1.default {
    constructor() {
        super();
        this.elements = {};
        ...
        this.elementFactories = [defaultElementFactory_1.default];
        this.eventListeners = {};
        this.store = {};
        (0, mobx_1.makeObservable)(this, {
            elements: mobx_1.observable.shallow,
            graph: mobx_1.observable.ref,
            viewConstraintsEnabled: mobx_1.observable,
            viewPaddingSettings: mobx_1.observable.ref,
            viewPadding: mobx_1.computed,
            store: mobx_1.observable.shallow,
            fromModel: mobx_1.action,
            setGraph: mobx_1.action
        });
    }
    get viewPadding() {
    ...
    fromModel(model, merge = true) {
        ...
        if (model.graph) {
            this.graph = this.createElement(types_1.ModelKind.graph, model.graph);
            if (!merge && oldGraph) {
        ...
        model.nodes &&
            model.nodes.forEach((n) => {
                idToElement[n.id] = n;
                this.createElement(types_1.ModelKind.node, n);
                validIds.push(n.id);
            });
        model.edges &&
            model.edges.forEach((e) => {
                idToElement[e.id] = e;
                this.createElement(types_1.ModelKind.edge, e);
                validIds.push(e.id);
            });
        // merge data
    ...
    toModel() {
        ...
        const nodes = this.getElements().filter((n) => (0, types_1.isNode)(n));
        const edges = this.getElements().filter((e) => (0, types_1.isEdge)(e));
        return {
            graph: graph.toModel(),
    ...
    setFitToScreenOnLayout(fitToScreen, padding = 80) {
        if (this.fitToScreenListener) {
            this.removeEventListener(types_1.GRAPH_LAYOUT_END_EVENT, this.fitToScreenListener);
        }
        if (fitToScreen) {
            this.fitToScreenListener = ({ graph }) => {
                graph.fit(padding);
            };
            this.addEventListener(types_1.GRAPH_LAYOUT_END_EVENT, this.fitToScreenListener);
            return;
        }
    }
