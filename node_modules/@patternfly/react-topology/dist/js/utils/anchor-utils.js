...
exports.getPolygonAnchorPoint = exports.getPathAnchorPoint = exports.getRectAnchorPoint = exports.getEllipseAnchorPoint = exports.getLinesIntersection = exports.distanceToPoint = void 0;
...
const getEllipseAnchorPoint = (center, width, height, reference) => {
    ...
    return new Point_1.default((center.x - x) * lenProportion + x, (center.y - y) * lenProportion + y);
};
exports.getEllipseAnchorPoint = getEllipseAnchorPoint;
...
exports.getRectAnchorPoint = getRectAnchorPoint;
const svgPointToPoint = (p) => new Point_1.default(p.x, p.y);
...
const getLinesIntersection = (line1, line2) => {
    ...
    const intersection = new Point_1.default(xValue / denominator, yValue / denominator);
...
const getPathIntersectionPoint = (pathNode, line) => {
    ...
    for (let i = 0; i < numSegments; i++) {
        ...
        const intersectPoint = (0, exports.getLinesIntersection)([svgPointToPoint(pos1), svgPointToPoint(pos2)], line);
        if (intersectPoint) {
    ...
    return new Point_1.default(pathBox.x + pathBox.width / 2, pathBox.y + pathBox.height / 2);
};
const getPathClosestPoint = (pathNode, reference) => {
    ...
    for (let scanLength = 0; scanLength <= pathLength; scanLength += precision) {
        ...
        const scanDistance = (0, exports.distanceToPoint)(svgPointToPoint(scan), reference);
    ...
    while (precision > 0.5) {
        ...
        const beforeDistance = (0, exports.distanceToPoint)(svgPointToPoint(before), reference);
        if (beforeLength >= 0 && beforeDistance < bestDistance) {
            ...
            const afterDistance = (0, exports.distanceToPoint)(svgPointToPoint(after), reference);
...
const getPathAnchorPoint = (pathNode, reference, useClosestPathPoint = false) => {
    ...
    const pathBox = pathNode.getBBox();
    const pathCenter = new Point_1.default(pathBox.x + pathBox.width / 2, pathBox.y + pathBox.height / 2);
    return getPathIntersectionPoint(pathNode, [reference, pathCenter]);
};
exports.getPathAnchorPoint = getPathAnchorPoint;
const getPolygonAnchorPoint = (polygonNode, reference) => {
    const polygonBox = polygonNode.getBBox();
    const polygonCenter = new Point_1.default(polygonBox.x + polygonBox.width / 2, polygonBox.y + polygonBox.height / 2);
    const { points } = polygonNode;
    let bestPoint = polygonCenter;
    let bestDistance = Infinity;
    for (let i = 0; i < points.length; i++) {
        const intersectPoint = (0, exports.getLinesIntersection)([svgPointToPoint(points[i]), svgPointToPoint(points[i === points.length - 1 ? 0 : i + 1])], [polygonCenter, reference]);
        if (intersectPoint) {
            const intersectDistance = (0, exports.distanceToPoint)(intersectPoint, reference);
...
exports.getPolygonAnchorPoint = getPolygonAnchorPoint;
