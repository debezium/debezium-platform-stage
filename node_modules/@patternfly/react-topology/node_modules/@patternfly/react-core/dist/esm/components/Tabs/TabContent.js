...
const TabContentBase = (_a) => {
    ...
    if (children || child) {
        ...
        return (React.createElement(TabsContextConsumer, null, ({ variant }) => {
            const variantClass = variantStyle[variant];
            return (React.createElement("section", Object.assign({ ref: innerRef, hidden: children ? null : child.props.eventKey !== activeKey, className: children
                    ? css(styles.tabContent, className, variantClass)
                    : css(styles.tabContent, child.props.className, variantClass), id: children ? id : `pf-tab-section-${child.props.eventKey}-${id}`, "aria-label": ariaLabel, "aria-labelledby": labelledBy, role: "tabpanel", tabIndex: 0 }, getOUIAProps('TabContent', ouiaId, ouiaSafe), props), children || child.props.children));
        }));
    }
    return null;
};
