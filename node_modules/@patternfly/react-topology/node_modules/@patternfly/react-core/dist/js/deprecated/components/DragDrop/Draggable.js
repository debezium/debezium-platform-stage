...
function resetDroppableItem(droppableItem) {
    ...
    droppableItem.node.classList.remove(drag_drop_1.default.modifiers.dragging);
    droppableItem.node.classList.remove(drag_drop_1.default.modifiers.dragOutside);
...
const Draggable = (_a) => {
    var { className, children, style: styleProp = {}, hasNoWrapper = false } = _a, props = tslib_1.__rest(_a, ["className", "children", "style", "hasNoWrapper"]);
    ...
    const { onDrag, onDragMove, onDrop } = React.useContext(DragDrop_1.DragDropContext);
    ...
    const onMouseMoveWhileDragging = (ev, droppableItems, blankDivRect) => {
        ...
        droppableItems.forEach((droppableItem) => {
            ...
            if (overlaps(ev, rect)) {
                ...
                node.classList.remove(drag_drop_1.default.modifiers.dragOutside);
                ...
                node.classList.add(drag_drop_1.default.modifiers.dragging);
                node.classList.add(drag_drop_1.default.modifiers.dragOutside);
            }
        });
    ...
    const onDragStart = (ev) => {
        ...
        const droppableItems = droppableNodes.reduce((acc, cur) => {
            cur.classList.add(drag_drop_1.default.modifiers.dragging);
    ...
    const childProps = Object.assign({ 'data-pf-draggable-zone': isDragging ? null : zone, draggable: true, className: (0, react_styles_1.css)(drag_drop_1.default.draggable, isDragging && drag_drop_1.default.modifiers.dragging, !isValidDrag && drag_drop_1.default.modifiers.dragOutside, className), onDragStart,
        onTransitionEnd,
        style }, props);
