...
class DualListSelector extends React.Component {
    ...
    constructor(props) {
        ...
        this.onFilterUpdate = (newFilteredOptions, paneType, isSearchReset) => {
            ...
            if (paneType === 'available') {
                ...
                if (isTree) {
                    this.setState({
                        availableTreeFilteredOptions: (0, treeUtils_1.flattenTreeWithFolders)(newFilteredOptions)
                    });
                }
                else {
                    this.setState({
                ...
                if (isTree) {
                    this.setState({
                        chosenTreeFilteredOptions: (0, treeUtils_1.flattenTreeWithFolders)(newFilteredOptions)
                    });
                }
                else {
                    this.setState({
        ...
        this.addAllTreeVisible = (event) => {
            this.setState((prevState) => {
                const movedOptions = prevState.availableTreeFilteredOptions ||
                    (0, treeUtils_1.flattenTreeWithFolders)(prevState.availableOptions);
                const newAvailable = prevState.availableOptions
                    .map((opt) => Object.assign({}, opt))
                    .filter((item) => (0, treeUtils_1.filterRestTreeItems)(item, movedOptions));
                const currChosen = (0, treeUtils_1.flattenTree)(prevState.chosenOptions);
                const nextChosenOptions = currChosen.concat(movedOptions);
                const newChosen = this.createMergedCopy()
                    .map((opt) => Object.assign({}, opt))
                    .filter((item) => (0, treeUtils_1.filterTreeItemsWithoutFolders)(item, nextChosenOptions));
                this.props.addAll && this.props.addAll(newAvailable, newChosen);
                this.props.onListChange && this.props.onListChange(event, newAvailable, newChosen);
                return {
                    chosenOptions: newChosen,
        ...
        this.addTreeSelected = (event) => {
            this.setState((prevState) => {
                ...
                const newAvailable = prevState.availableOptions
                    .map((opt) => Object.assign({}, opt))
                    .filter((item) => (0, treeUtils_1.filterRestTreeItems)(item, prevState.availableTreeOptionsChecked));
        ...
        this.removeAllTreeVisible = (event) => {
            this.setState((prevState) => {
                const movedOptions = prevState.chosenTreeFilteredOptions ||
                    (0, treeUtils_1.flattenTreeWithFolders)(prevState.chosenOptions);
                const newChosen = prevState.chosenOptions
                    .map((opt) => Object.assign({}, opt))
                    .filter((item) => (0, treeUtils_1.filterRestTreeItems)(item, movedOptions));
                const currAvailable = (0, treeUtils_1.flattenTree)(prevState.availableOptions);
                const nextAvailableOptions = currAvailable.concat(movedOptions);
                const newAvailable = this.createMergedCopy()
                    .map((opt) => Object.assign({}, opt))
                    .filter((item) => (0, treeUtils_1.filterTreeItemsWithoutFolders)(item, nextAvailableOptions));
                this.props.removeAll && this.props.removeAll(newAvailable, newChosen);
                this.props.onListChange && this.props.onListChange(event, newAvailable, newChosen);
                return {
                    chosenOptions: newChosen,
        ...
        this.removeTreeSelected = (event) => {
            this.setState((prevState) => {
                ...
                const newChosen = prevState.chosenOptions
                    .map((opt) => Object.assign({}, opt))
                    .filter((item) => (0, treeUtils_1.filterRestTreeItems)(item, prevState.chosenTreeOptionsChecked));
        ...
        this.onTreeOptionCheck = (evt, isChecked, itemData, isChosen) => {
            ...
            if (isChosen) {
                if (chosenTreeFilteredOptions) {
                    panelOptions = chosenOptions
                        .map((opt) => Object.assign({}, opt))
                        .filter((item) => (0, treeUtils_1.filterTreeItemsWithoutFolders)(item, chosenTreeFilteredOptions));
                }
                else {
                    panelOptions = chosenOptions;
                }
                ...
                if (availableTreeFilteredOptions) {
                    panelOptions = availableOptions
                        .map((opt) => Object.assign({}, opt))
                        .filter((item) => (0, treeUtils_1.filterTreeItemsWithoutFolders)(item, availableTreeFilteredOptions));
                }
                else {
                    panelOptions = availableOptions;
                }
            ...
            const checkedOptionTree = panelOptions
                .map((opt) => Object.assign({}, opt))
                .filter((item) => (0, treeUtils_1.filterTreeItems)(item, [itemData.id]));
            const flatTree = (0, treeUtils_1.flattenTreeWithFolders)(checkedOptionTree);
    ...
    render() {
        ...
        const availableOptionsStatusToDisplay = availableOptionsStatus ||
            (isTree
                ? `${(0, treeUtils_1.filterFolders)(availableOptions, availableTreeOptionsChecked)
                    .length} of ${(0, treeUtils_1.flattenTree)(availableOptions).length} items selected`
        ...
        const chosenOptionsStatusToDisplay = chosenOptionsStatus ||
            (isTree
                ? `${(0, treeUtils_1.filterFolders)(chosenOptions, chosenTreeOptionsChecked).length} of ${(0, treeUtils_1.flattenTree)(chosenOptions).length} items selected`
        ...
        return (React.createElement(DualListSelectorContext_1.DualListSelectorContext.Provider, { value: { isTree } },
            React.createElement(helpers_1.GenerateId, null, (randomId) => (React.createElement("div", Object.assign({ className: (0, react_styles_1.css)(dual_list_selector_1.default.dualListSelector, className), id: id || randomId }, props), children === '' ? (React.createElement(React.Fragment, null,
                ...
                React.createElement(DualListSelectorControlsWrapper_1.DualListSelectorControlsWrapper, { "aria-label": controlsAriaLabel },
                    ...
                    React.createElement(DualListSelectorControl_1.DualListSelectorControl, { onClick: isTree ? this.removeTreeSelected : this.removeSelected, isDisabled: (isTree ? chosenTreeOptionsChecked.length === 0 : chosenOptionsSelected.length === 0) ||
