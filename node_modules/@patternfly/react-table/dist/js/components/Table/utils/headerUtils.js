...
const selectableTransforms = ({ onSelect, canSelectAll }) => [
    ...(onSelect
        ? [
            {
                title: '',
                transforms: (canSelectAll && [decorators_1.selectable]) || null,
                cellTransforms: [decorators_1.selectable]
            }
        ]
        : [])
];
        ...
        ? [
            {
                ...
                transforms: onSort && canSortFavorites
                    ? [
                        transformers_1.scopeColTransformer,
                        (0, decorators_1.sortableFavorites)({
                            onSort,
                    ...
                    : [transformers_1.emptyTD],
                cellTransforms: [decorators_1.favoritable]
            }
        ]
        : [])
];
        ...
        ? [
            {
                ...
                cellTransforms: [(0, decorators_1.cellActions)(actions, actionResolver, areActionsDisabled)]
            }
        ]
        : [])
];
...
const collapsibleTransforms = (header, { onCollapse, canCollapseAll, firstUserColumnIndex }) => [
    ...(onCollapse
        ? [
            {
                title: '',
                transforms: (canCollapseAll && [decorators_1.collapsible]) || null,
                cellTransforms: [decorators_1.collapsible, (0, decorators_1.expandedRow)(header.length, firstUserColumnIndex)]
            }
        ]
        : [])
];
...
const expandContent = (header, extra) => {
    ...
    return header.map((cell) => {
        const parentIdCell = addAdditionalCellTranforms(cell, transformers_1.parentId);
        return addAdditionalCellTranforms(parentIdCell, (0, decorators_1.expandedRow)(header.length));
    });
};
/**
...
const rowEditTransforms = ({ onRowEdit }) => [
    ...(onRowEdit
        ? [
            {
                title: '',
                cellTransforms: [decorators_1.editable]
            }
        ]
        : [])
];
