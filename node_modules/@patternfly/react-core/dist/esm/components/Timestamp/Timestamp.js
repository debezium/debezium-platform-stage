...
import styles from '@patternfly/react-styles/css/components/Timestamp/timestamp.mjs';
...
export const Timestamp = (_a) => {
    ...
    const timestamp = (React.createElement("span", Object.assign({ className: css(styles.timestamp, tooltip && styles.modifiers.helpText, className) }, (tooltip && { tabIndex: 0 }), propsWithoutDateTime),
        React.createElement("time", { className: `${styles.timestamp}__text`, dateTime: dateTime || new Date(date).toISOString() }, !children ? defaultDisplay : children)));
    return tooltip ? (React.createElement(Tooltip, Object.assign({ content: tooltip.variant === TimestampTooltipVariant.default ? createUTCContent(tooltip.suffix) : tooltip.content }, tooltip.tooltipProps), timestamp)) : (timestamp);
};
