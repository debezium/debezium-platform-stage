...
const CreationFlowTransform: React.FC<CreationFlowTransformProps> = ({
  ...
  const transformGroupNode = useMemo(() => {
    return {
      ...
      data: {
        ...
        onToggleDrawer1: onToggleDrawer,
      },
      position: { x: 260, y: 45 },
      style: {
        width: 130,
  ...
  const handleProcessor = useCallback(() => {
    setNodes((prevNodes: any) => {
      return [
        ...prevNodes.filter((node: any) => node.id !== "debezium"),
        addTransformNode,
        transformGroupNode,
      ];
    });
  ...
  //   setNodes((prevNodes: any) => {
  //     return [
  //       ...prevNodes.filter((node: any) => node.id.includes("transform")),
  ...
  const handleAddTransform = useCallback(
    (transform: TransformData) => {
      const noOfTransformNodes = nodes.filter((node: any) => {
